...
class Preprocess:
    ...
    def create_folder_environment(self):
        ...
        destination_path = os.path.join(os.getcwd(), "preprocessing_files")

        if os.path.exists(destination_path):
            files_in_destination = os.listdir(destination_path)
            if not config.SKIP_QUESTIONS and files_in_destination:
                response = input("║  The 'preprocessing_files' folder already exists and its NOT empty.\n"
    ...
    def preprocess_dataset(self):
        print("====> Down sampling data and moving it to the '/preprocessing_files' folder")
        source_path = self.path
        destination_path = os.path.join(os.getcwd(), "preprocessing_files")
        folders = [f for f in os.listdir(source_path) if os.path.isdir(os.path.join(source_path, f))]
